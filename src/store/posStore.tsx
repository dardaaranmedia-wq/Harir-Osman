...
export const POSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createOrder = (
    ...
    waiter?: string, 
    isQR: boolean = false,
    customerType: "Dine-In" | "Takeaway" | "Delivery" = "Dine-In"
  ): Order => {
    const isTakeaway = customerType === "Takeaway";
    const targetTable = isTakeaway ? null : tables.find(t => t.tableId === tableId || t.id === tableId);
    const tblId = isTakeaway ? "TAKEAWAY" : (targetTable ? targetTable.tableId : `LUNA-T${tableId}`);
    const tblName = isTakeaway ? "Takeaway (To Go)" : (targetTable ? targetTable.name : `Table ${tableId}`);
  ...
  const cancelOrder = (orderId: string, reason?: string) => {
    ...
    setOrders(prev => prev.map(o => {
      if (o.id === orderId) {
        ...
        return { 
          ...
          cancelReason: reason || "Command void",
          updatedAt: new Date().toISOString() 
        };
      }
      return o;
    ...
    addNotification("SYSTEM", `Order Cancelled`, `Order ${orderNum} has been cancelled.${reason ? ` Reason: ${reason}` : ""}`);
  };
